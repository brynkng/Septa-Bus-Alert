class TravelTimeCalculator
	...
	def self.get_next_arrivals_for_all_buses(route_direction, desiredStop)

	end

	def self.get_next_arrival(route_direction, desiredStop)

		travel_time = nil
		bus = nil

		travel_time, bus = calculate_travel_time_until_next_bus(route_direction, desiredStop)
		...
		unless travel_time > 0
			...
			opposite_route_direction = RouteDirection.where(
					'route_id = ? and direction_id = ?',
					route_direction.route_id,
					route_direction.direction_id == 0 ? 1 : 0
			).first

			last_stop_on_opposite_direction = SimplifiedStop.where('route_direction_id = ?', opposite_route_direction.id).order('stop_sequence DESC').limit(1).first
			first_stop_on_route = SimplifiedStop.where('route_direction_id = ?', route_direction.id).order('stop_sequence').limit(1).first
			...
			first_stop_travel_time = self.get_travel_time_between_stops(first_stop_on_route, desiredStop)
	...
	def self.calculate_travel_time_until_next_bus(route_direction, desiredStop)
		buses = ::SeptaApiWrapper.get_next_buses_on_route(route_direction.route_short_name, route_direction.direction_name, desiredStop.stop_lat, desiredStop.stop_lon)
		travel_time = 0
		bus_number = 0
		bus = nil
		...
		while travel_time < 1 and bus_number < buses.count
			...
			travel_time_between_stops = get_travel_time_between_stops(stop_near_bus, desiredStop)
			#puts 'travel time between stops: ' + travel_time_between_stops.to_s
			offset = (bus.offset.to_i * 60)
			#puts 'offset: ' + offset.to_s
			travel_time = travel_time_between_stops - offset
	...
	def self.get_travel_time_between_stops(startStop, endStop)
		travel_time = 0
		#puts 'start stop: ' + startStop.stop_id.to_s
		#puts 'end stop: ' + endStop.stop_id.to_s

		stop_slots = get_stop_slots_between_stops(startStop, endStop)
	...
	#
	# Params:
	# +startStop+:: +SimplifiedStop+ object
	# +endStop+:: +SimplifiedStop+ object
	def self.get_stop_slots_between_stops(startStop, endStop)

		stop_ids = SimplifiedStop.where(
				'route_id = ? AND route_direction_id = ? AND stop_sequence BETWEEN ? AND ?',
				startStop.route_id,
				startStop.route_direction_id,
				startStop.stop_sequence,
				endStop.stop_sequence
